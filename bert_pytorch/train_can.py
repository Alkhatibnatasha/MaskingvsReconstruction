...
from bert_pytorch.dataset import CANDataset, WordVocab
...
class Trainer():
    def __init__(self, options):
        ...
        self.can_freq = options["can_freq"]
    ...
    def train(self):
        ...
        id_train, id_valid, time_train, time_valid = generate_train_valid(self.train_data, window_size=self.window_size,
        ...
        train_dataset = CANDataset(id_train,time_train, vocab, seq_len=self.seq_len,
                                    corpus_lines=self.corpus_lines, on_memory=self.on_memory, mask_ratio=self.mask_ratio)
            
        print("\nLoading valid Dataset")

        valid_dataset = CANDataset(id_valid, time_valid, vocab, seq_len=self.seq_len, on_memory=self.on_memory, mask_ratio=self.mask_ratio)
        ...
        bert = BERT(len(vocab), max_len=self.max_len, hidden=self.hidden, n_layers=self.layers, attn_heads=self.attn_heads,
                    is_id=self.is_id, is_time=self.is_time)
        ...
        self.trainer = BERTTrainer(bert, len(vocab), train_dataloader=self.train_data_loader,    valid_dataloader=self.valid_data_loader,
                              ...
                              with_cuda=self.with_cuda, cuda_devices=self.cuda_devices, can_freq=self.can_freq,
                              is_id=self.is_id, is_time=self.is_time,
                              hypersphere_loss=self.hypersphere_loss)
        
        
        
        self.start_iteration(surfix_log="can")
            
        self.plot_train_valid_loss("_can")

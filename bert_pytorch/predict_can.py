...
from bert_pytorch.dataset import CANDataset
...
def compute_anomaly(results, params, seq_threshold=0.5):
    ...
    for seq_res in results:
        ...
        if (is_id and seq_res["undetected_tokens"] > seq_res["masked_tokens"] * seq_threshold) or \
...
class Predictor():
    ...
    def detect_id_anomaly(self, masked_output, masked_label):
    ...
    def generate_test(output_dir, file_name, window_size, adaptive_window, seq_len, scale, min_len):
        ...
        with open(output_dir + file_name, "r") as f:
            for idx, line in tqdm(enumerate(f.readlines())):
                #if idx > 40: break
                id_seq, tim_seq = fixed_window(line, window_size,
        ...
        test_len = list(map(len, id_seqs))
        test_sort_index = np.argsort(-1 * np.array(test_len))

        id_seqs = id_seqs[test_sort_index]
        tim_seqs = tim_seqs[test_sort_index]

        print(f"{file_name} size: {len(id_seqs)}")
        return id_seqs, tim_seqs
    ...
    def helper(self, model, output_dir, file_name, vocab, scale=None, error_dict=None):
        ...
        total_dist = []
        output_cls = []
        id_test, time_test = self.generate_test(output_dir, file_name, self.window_size, self.adaptive_window, self.seq_len, scale, self.min_len)

        # use 1/10 test data
        if self.test_ratio != 1:
            num_test = len(id_test)
            ...
            id_test, time_test = id_test[rand_index], time_test[rand_index]


        seq_dataset = CANDataset(id_test, time_test, vocab, seq_len=self.seq_len,
        ...
        for idx, data in enumerate(data_loader):
            ...
            mask_lm_output, mask_tm_output = result["id_output"], result["time_output"]
            ...
            for i in range(len(data["bert_label"])):
                seq_results = {"num_error": 0,
                               "undetected_tokens": 0,
                               "masked_tokens": 0,
                               "total_id": torch.sum(data["bert_input"][i] > 0).item(),
                               "deepSVDD_label": 0
                               }
                ...
                if self.is_id:
                    num_undetected, output_seq = self.detect_id_anomaly(
    ...
    def predict(self):
        ...
        test_normal_results, test_normal_errors = self.helper(model, self.output_dir + "train_valid_test/test/ambient/", "correlated_signal_attack_1", vocab, scale, error_dict)

        print("test abnormal predicting")
        test_abnormal_results, test_abnormal_errors = self.helper(model, self.output_dir+"train_valid_test/test/attacks/", "correlated_signal_attack_1", vocab, scale, error_dict)
        ...
        params = {"is_id": self.is_id, "is_time": self.is_time, "hypersphere_loss": self.hypersphere_loss,

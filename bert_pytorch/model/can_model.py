...
class BERTCAN(nn.Module):
    ...
    def __init__(self, bert: BERT, vocab_size):
        ...
        super().__init__()
        self.bert = bert
        self.mask_lm = MaskedCANModel(self.bert.hidden, vocab_size)
        self.time_lm = TimeCANModel(self.bert.hidden)
        # self.fnn_cls = LinearCLS(self.bert.hidden)
        #self.cls_lm = LogClassifier(self.bert.hidden)
        self.result = {"id_output": None, "time_output": None, "cls_output": None, "cls_fnn_output": None}

    def forward(self, x, time_info):
        x = self.bert(x, time_info=time_info)

        self.result["id_output"] = self.mask_lm(x)
...
class MaskedCANModel(nn.Module):
    """

...
from ..model import BERTCAN, BERT
...
class BERTTrainer:
    ...
    def __init__(self, bert: BERT, vocab_size: int,
                 ...
                 with_cuda: bool = True, cuda_devices=None, can_freq: int = 10, is_id=True, is_time=False,
        ...
        self.model = BERTCAN(bert, vocab_size).to(self.device)
    ...
    def iteration(self, epoch, data_loader, start_train):
        ...
        for i, data in data_iter:
            ...
            mask_lm_output, mask_time_output = result["id_output"], result["time_output"]

            # 2-2. NLLLoss of predicting masked token word ignore_index = 0 to ignore unmasked tokens
            mask_loss = torch.tensor(0) if not self.is_id else self.criterion(mask_lm_output.transpose(1, 2), data["bert_label"])
            total_id_loss += mask_loss.item()
        ...
        print(f"ID loss: {total_id_loss/totol_length}, hyper loss: {total_hyper_loss/totol_length}\n")
